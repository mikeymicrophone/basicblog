	...
	function joinAdd(&$entity, $type='INNER', $alias = '') {
		...
		foreach($fk as $f=>$k) {
			...
			if($k['type'] == 'one-to-many' && $k['class'] == $entity->oTable->class) {
				...
				if($alias == '') {
					$joinString .= " $type JOIN {$entity->oTable->tablename} on({$this->oTable->tablename}.{$tk['column']}={$entity->oTable->tablename}.{$ek['column']}) ";
				} else {
					$joinString .= " $type JOIN {$entity->oTable->tablename} as $alias on({$this->oTable->tablename}.{$tk['column']}=$alias.{$ek['column']}) ";
				}
				break;
			}
			...
			if($k['type'] == 'many-to-one') {
				if($k['class'] == $entity->oTable->class) {
					...
					if($alias == '') {
						$joinString .= " $type JOIN {$entity->oTable->tablename} on({$this->oTable->tablename}.{$k['column']}={$entity->oTable->tablename}.{$linkOn['column']}) ";
					} else {
						$joinString .= " $type JOIN {$entity->oTable->tablename} as $alias on({$this->oTable->tablename}.{$k['column']}=$alias.{$linkOn['column']}) ";
					}
					break;
				}
			}
				...
				if($k['class'] == $entity->oTable->class) {
					...
					foreach($x as $ename => $eprop) {
						if(isset($eprop['table']) && $eprop['table'] == $k['table'] && $entity->oTable->class == $k['class']) {
							...
							$joinString = " $type JOIN  {$k['table']} on({$k['table']}.{$tfp['column']} = $table.{$tfp['column']}) ";
							$joinString .= " $type JOIN $etable on({$eprop['table']}.{$efp['column']} = $etable.{$efp['column']}) ";
			...
			foreach($fk as $f=>$k) {
				if($k['type'] == 'many-to-one') {
					if($k['class'] == $this->oTable->class) {
						...
						$joinString = " $type JOIN {$entity->oTable->tablename} on({$this->oTable->tablename}.{$x['column']}={$entity->oTable->tablename}.{$k['column']}) ";
						break;
					}
				}
			}
